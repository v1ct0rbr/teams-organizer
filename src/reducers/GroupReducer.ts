...
export function newGroupName(dataName: string): Group {
  return {
    id: uuid.v4(),
    name: dataName,
    teamA: { name: "TIME A", participants: [] },
    teamB: { name: "TIME B", participants: [] },
  } as Group;
}
  ...
  switch (action.type) {
    ...
    case "ADD_NEW_GROUP": {
      ...
      const grupoExistente =
        state.groups &&
        state.groups.findIndex((g) => g.name === action.payload.name);
      if (grupoExistente)
        return {
          ...state,
    ...
    case "UPDATE_TEAM": {
      ...
      state.groups.forEach((g) => {
        if (g.id === action.payload.activeGroupId) {
          group =
            g.teamA.name === action.payload.team.name
              ? { ...g, teamA: action.payload.team }
              : { ...g, teamB: action.payload.team };
          groups.push(group);
        } else {
          group = { ...g };
          groups.push(group);
      ...
      return { ...state, groups, activeGroup: group };
    }
    default:
      return state;
  }
